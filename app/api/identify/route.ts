...
export async function POST(request: Request) {
  let input;
  try {
    input = await readJson(request, 7 * 1024 * 1024);
  } catch (error) {
  ...
  if (!key)
    return Response.json(
      {
        error:
          "Image identification needs OPENAI_API_KEY in the project .env. You can still search by title.",
      },
      { status: 503 },
    );
  try {
        ...
        input: [
          {
            ...
            content: [
              {
                type: "input_text",
                text: "Suggest a searchable title for this clothing item.",
              },
    ...
    if (!response.ok)
      return Response.json(
        {
          error: `Image provider returned ${response.status}. Check the API key, model access and quota.`,
        },
        { status: 502 },
      );
    ...
    const query = (data.output ?? [])
      ...
      .trim()
      .slice(0, 180);
    if (!query) throw new Error("No usable search terms returned");
    return Response.json(
      { query },
    ...
    return Response.json(
      {
        error:
          "Image identification could not finish. Try again or enter a title.",
      },
      { status: 502 },
    );
  }
